...
# Read in DiffBind, plot 
fs = [
    "DBA_BRD4_dBET_vs_BRD4_DMSO.csv",
    "DBA_MTHFD1_dBET_vs_MTHFD1_DMSO.csv",
    "DBA_BRD4_MTHFD1KO_vs_BRD4_WT.csv",
    "DBA_MTHFD1_MTHFD1KO_vs_MTHFD1_WT.csv"
]

res = pd.DataFrame()
for f in fs:
    df = pd.read_csv(f)
    df['comparison_name'] = "_".join(f.split("_")[1:5])
    res = res.append(df)
    ...
    fig.savefig(os.path.join("results", "diffBind.volcano.{}.scatter.new.svg".format(f)), dpi=300, bbox_inches="tight")
    ...
    fig.savefig(os.path.join("results", "diffBind.scatterplot.{}.scatter.new.svg".format(f)), dpi=300, bbox_inches="tight")


res = res[~res['Chr'].str.contains("_")]
res = res.set_index(['Chr', 'Start', 'End'])

res.to_csv(os.path.join("results", "diffBind.results.csv"))
...
grid = sns.clustermap(res3, cmap="RdBu_r", center=0, metric="correlation", rasterized=True, yticklabels=False, cbar_kws={"label": "log2(Fold-change)"})
grid.savefig(os.path.join("results", "diffBind.log_fold_changes.signficant.heatmap.svg"), dpi=300, bbox_inches="tight")

# signed p-values
res4 = res.pivot_table(index=['Chr', 'Start', 'End'], columns='comparison_name', values="p-value")
...
grid2.savefig(os.path.join("results", "diffBind.pvalue.signficant.heatmap.svg"), dpi=300, bbox_inches="tight")
...
grid3.savefig(os.path.join("results", "diffBind.concentration.signficant.heatmap.svg"), dpi=300, bbox_inches="tight")
order = ["Conc_BRD4_WT", "Conc_BRD4_MTHFD1KO", "Conc_BRD4_DMSO", "Conc_BRD4_dBET", "Conc_MTHFD1_WT", "Conc_MTHFD1_MTHFD1KO", "Conc_MTHFD1_DMSO", "Conc_MTHFD1_dBET"]
p = res7.loc[:, order]
...
grid3.savefig(os.path.join("results", "diffBind.concentration.signficant.heatmap.sorted.svg"), dpi=300, bbox_inches="tight")
grid3 = sns.clustermap(
    p.loc[:, p.columns.str.contains("DMSO|dBET")], metric="correlation", figsize=(4, 4),
    rasterized=True, yticklabels=False, cbar_kws={"label": "Concentration"},
    col_cluster=False, robust=True)
grid3.savefig(os.path.join("results", "diffBind.concentration.signficant.heatmap.sorted.dmso_dbet.svg"), dpi=300, bbox_inches="tight")
grid3 = sns.clustermap(
    p.loc[:, ~p.columns.str.contains("DMSO|dBET")], metric="correlation", figsize=(4, 4),
    rasterized=True, yticklabels=False, cbar_kws={"label": "Concentration"},
    col_cluster=False, robust=True)
grid3.savefig(os.path.join("results", "diffBind.concentration.signficant.heatmap.sorted.wt_ko.svg"), dpi=300, bbox_inches="tight")
...
grid3 = sns.clustermap(
    ...
    rasterized=True, yticklabels=False, cbar_kws={"label": "Concentration"},
    col_cluster=False, robust=True)
grid3.savefig(os.path.join("results", "diffBind.concentration.signficant.heatmap.sorted.dmso_dbet-specific_regions.svg"), dpi=300, bbox_inches="tight")
...
sns.barplot(
    ...
    y="variable", x="value", orient="horiz",
    ax=axis[1])
fig.savefig(os.path.join("results", "diffBind.concentration.signficant.barplot.svg"), dpi=300, bbox_inches="tight")

from scipy.stats import mannwhitneyu
r = list()
for a in res7.columns:
    for b in res7.columns:
        r.append([a, b, *mannwhitneyu(res7[a], res7[b])])

r = pd.DataFrame(r, columns=['a', 'b', 'stat', 'p']).pivot_table(index='a', columns='b', values='p')
r.to_csv(os.path.join("results", "diffBind.signficance.csv"))
